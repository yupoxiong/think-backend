...
use yupoxiong\addons\middleware\Addons;
...
class Service extends \think\Service
{
    ...
    public function register()
    {
        ...
        // 加载系统语言包
        Lang::load([
            $this->app->getRootPath() . '/vendor/yupoxiong/think-addons/src/lang/zh-cn.php'
        ]);
        // 自动载入插件
        $this->autoload();
        // 加载插件事件
        ...
        $this->loadService();
        // 绑定插件容器
        $this->app->bind('addons', __CLASS__);
    }
    ...
    public function boot()
    {
        $this->registerRoutes(function (Route $route) {
            // 路由脚本
            $execute = '\\yupoxiong\\addons\\Route::execute';
    ...
    private function autoload()
    {
        ...
        // 读取插件目录及钩子列表
        $base = get_class_methods(\yupoxiong\addons\Addons::class);
        // 读取插件目录中的php文件
